        ...
        if to in wait['ROM']:
            ...
            for m in sort:
                ...
                if m == sort[0]:
                    ...
                    msgas+= '\n\n{}. @!{}x{}'.format(no,len(moneys[m][0]),has)
            self.sendMentions(to, msgas,'', h)
            del wait['ROM'][to]
        else:
            try:
                msgas = 'Sorry @!In {} nothink get a mention'.format(self.getGroup(to).name)
                self.sendMentions(to, msgas,' 「 Mention Me 」\n', [self.getProfile().mid])
            except:
                msgas = 'Sorry @!In Chat @!nothink get a mention'
                self.sendMentions(to, msgas,' 「 Mention Me 」\n', [self.getProfile().mid,to])

    @loggedIn
    def lurkr(self,to,read,msg):
        msg.text = self.mycmd(msg.text,read)
        if 'MENTION' in msg.contentMetadata.keys()!=None:
            key = eval(msg.contentMetadata["MENTION"])
            targets = key["MENTIONEES"][0]["M"]
            if targets in read['readPoints']:
                chiya = []
                for rom in read["lurkp"][to][targets].items():
                    chiya.append(rom[1])
                k = len(chiya)//100
                for a in range(k+1):
                    if a == 0:self.mentionmention(to=to,read=read,text='',dataMid=chiya[:100],pl=0,ps='╭「 Lurkers 」─',pg='SIDERMES',pt=chiya)
                    else:self.mentionmention(to=to,read=read,text='',dataMid=chiya[a*100 : (a+1)*100],pl=a*100,ps='├「 Lurkers 」─',pg='SIDERMES',pt=chiya)
                read['lurkt'][to][targets]  = {};read['lurkp'][to][targets] = {}
            else:self.sendMentions(to, " 「 Lurk 」\nLurk in @! not active",'',[targets])
        else:
            if msg.text.lower() == "lurk result":
                if to in read['readPoint']:
                    try:
                        self.anulurk(to,read)
                        read['setTime'][to]  = {}
